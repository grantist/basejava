...
- Реализуйте класс `com.javops.webapp.storage.ArrayStorage` (вставте код в пустые методы класса): работа с `com.javops.webapp.model.Resume` на основе массива (методы `clear, get, save, delete, getAll, size`)
   - Не используйте в решении коллекции, реализацию на их основе мы добавим позднее
   - Не меняйте синтаксис методов `com.javops.webapp.storage.ArrayStorage` и не меняйте класс `com.javops.webapp.model.Resume`. Нужет только код внутри готовых методов `com.javops.webapp.storage.ArrayStorage`
...
- Протестируйте вашу реализацию интерактивно с помощью `com.javops.webapp.storage.MainArray.main()`.
...
### Optional
- Модифицировать класс `com.javops.webapp.storage.ArrayStorage`: хранить все резюме в начале storage (без дырок null), чтобы не перебирать каждый раз все 10000 элементов.
```
...
```
- Посмотреть на класс `Arrays`. Там есть полезные вещи, которые могут упростить код `com.javops.webapp.storage.ArrayStorage`.
- Протестируйте реализацию, запустив com.javops.webapp.storage.MainArray.main(): в IDEA слева на полях зеленая стрелка.
